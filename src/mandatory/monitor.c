...
/*   Updated: 2024/08/29 07:32:43 by emgul            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../inc/philo.h"

static bool all_threads_running(pthread_mutex_t *mutex, long *threads, long philo_count)
{
	bool ret;
...
static bool philo_died(t_table *table, t_philo *philo)
{
	...
	if (get_bool(&philo->philo_mutex, &philo->is_full))
		return (false);
	elapsed_time = get_time_milisec() - get_long(&philo->philo_mutex, &philo->last_meal_time);
...
void *watch_dinner(void *data)
{
	...
	table = (t_table *)data;
	while (!all_threads_running(&table->table_mutex, &table->running_threads_count, table->philo_count))
		continue ;
	while (!get_bool(&table->table_mutex, &table->dinner_over))
	{
		...
		if (get_long(&table->table_mutex, &table->full_philo_count) == table->philo_count)
			set_bool(&table->table_mutex, &table->dinner_over, true);
	}
	return (NULL);
}
